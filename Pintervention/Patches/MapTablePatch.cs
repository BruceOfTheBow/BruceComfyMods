...
static class MapTablePatch {
  ...
  static bool ClaimOwnership(MapTable table) {
    ...
    Pintervention.LogInfo("Claimed table ownership to write name data.");
  ...
  static void LoadPlayerNames(ZDO zdo) {
    ...
    while (storedValue != string.Empty) {
      ...
      if (kvp.Equals(default(KeyValuePair<long, string>))) {
        Pintervention.LogWarning($"Unable to parse name at index {i} with value {storedValue}.");
        continue;
      }
  ...
  static bool IsNameOnTable(MapTable table, long pid, string name) {
    ...
    Pintervention.LogInfo($"Name {Player.m_localPlayer.GetPlayerName()} not found on map table.");
    return false;
  }
