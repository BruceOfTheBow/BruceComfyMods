...
public static class NameManager {
  public static Dictionary<long, string> PlayerNamesById { get; private set; } = [];
  ...
  public static Dictionary<long, string> GetNamesToWrite() {
    Dictionary<long, string> namesToWrite = [];
  ...
  public static void ReadNamesFromFile() {
    ...
    if (!File.Exists(GetFilename())) {
      LogInfo($"No saved names to load from {GetFilename()}.");
      return;
    }

    LogInfo($"Loading saved player names from {GetFilename()}");

    Dictionary<int, string> loadedNames = [];
